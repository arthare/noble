...
NobleBindings.prototype.onDiscover = function (status, address, addressType, connectable, advertisement, rssi) {
  if (this._scanServiceUuids === undefined && status !== "REVIVE") {
    return;
  }
  ...
  let discoveredHasScanServiceUuids = status === "REVIVE" || (this._scanServiceUuids.length === 0) ;

  if (!discoveredHasScanServiceUuids) {
    let i;
    ...
    for (i in serviceUuids) {
      discoveredHasScanServiceUuids = (this._scanServiceUuids.indexOf(serviceUuids[i]) !== -1);
