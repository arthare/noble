...
Gap.prototype.onHciLeAdvertisingReport = function (status, type, address, addressType, eir, rssi, data) {
  ...
  let dataInitial = advertisement.dataInitial;
  let dataScanrsp = advertisement.dataScanrsp || null;
  ...
  while ((i + 1) < eir.length) {
    ...
    switch (eirType) {
      ...
      case 0x09: // Complete Local Name»
        ...
        console.log("Our name is ", advertisement.localName);
        break;
  ...
  console.log("done interpreting ", eir);
  ...
  if (type === 0x04 || !connectable || (discoveryCount > 1 && !hasScanResponse) || process.env.NOBLE_REPORT_ALL_HCI_EVENTS) {
    this.emit('discover', status, address, addressType, connectable, advertisement, rssi, data, dataInitial, dataScanrsp);
  }
};
